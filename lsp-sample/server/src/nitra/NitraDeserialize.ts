...
retStack.push((buf,stack) => {
	...
	msg.changes = [];
		stack.push(...GetFileChangeArrayDeserializer(msg.changes, length));
	return stack;
});
...
retStack.push((buf,stack) => {
	let length = buf.readInt32LE(0);
	msg.completionList = [];
		stack.push(...GetCompletionElemArrayDeserializer(msg.completionList, length));
	return stack;
});
;
 return retStack;
...
retStack.push((buf,stack) => {
	let length = buf.readInt32LE(0);
	msg.items = [];
		stack.push(...GetObjectDescriptorArrayDeserializer(msg.items, length));
	return stack;
});
;
 return retStack;
